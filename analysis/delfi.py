...
#                     return
    ...
    def writeinfile(self, outputfile):
        with open(outputfile, 'w') as f:
            for x in range(1, self.numberofchrom):  # Start from chromosome 1
                for y in range(self.numberofbin):
                    num_reads_A = self.allinfoA[x, y]
                    num_reads_B = self.allinfoB[x, y]

                    line = (
                        f'chromosom:,{x},Bin Index:,{y},NUmber of reads in range {self.minsizeA} until {self.maxsizeA},{num_reads_A},'
                        f'NUmber of reads in range {self.maxsizeA} until {self.maxsizeB},{num_reads_B},'
                        f'Normalized A:,{num_reads_A / (num_reads_A + num_reads_B) if (num_reads_A + num_reads_B) != 0 else 0},'
                        f'Normalized B:,{num_reads_B / (num_reads_A + num_reads_B) if (num_reads_A + num_reads_B) != 0 else 0},'
                        f'Ratio,{(num_reads_A / (num_reads_A + num_reads_B)) / (num_reads_B / (num_reads_A + num_reads_B)) if (num_reads_B != 0) else 0}\n'
                    )
                    f.write(line)
...
def main(argv):      
    ...
    inputfile = argv[6]
    outputfile = inputfile+'_delfi_frag.csv'
